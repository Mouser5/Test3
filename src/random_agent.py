...
def run_visual_match():
    ...
    try:
        while not game.is_game_over():
            ...
            if not action:
                view.print_message(f"У игрока {curr_p} нет легальных ходов!", is_error=True)
                break
            ...
            if turn_count % 10 == 0 or rev_gold is not None or "Обвал" in msg:
                ...
                if msg: view.print_message(f"> {msg}")
                if rev_gold: view.print_message(f"✨ НАЙДЕНО ЗОЛОТО: {rev_gold} ✨")
                view.print_board(game.state)
                time.sleep(0.3)
