        ...
        if choice == '1':
            try:
                ...
                if isinstance(tpl, PathCardTemplate) or (
                    ...
                    if isinstance(tpl, PathCardTemplate):
                        action = ActionBuild(template_id=t_id, x=x, y=y, is_rotated_180=rot)
                    else:
                    ...
                    action = ActionPlayPlayerUtility(template_id=t_id, target_player_id=t_target)
                else:
                    continue
            ...
            try:
                ...
                if eq_choice in eq_map:
                    ...
                    action = ActionDiscard(templates=templates, repair_equipment=eq_map[eq_choice])
                    success, msg, _ = game.step(action)
                    if success:
                        view.print_message(msg)
                    else:
