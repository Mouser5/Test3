...
def interactive_loop(game: Game):
    while True:
        print("\n" + "=" * 50)
        game.print_state()  # Использует state из онтологии

        hand = game.state.current_player.hand  # Кортеж вместо списка
        print(f"\nРука Игрока {game.state.current_player_id}:")
        ...
        try:
            ...
            if choice.isdigit():
                ...
                if 0 <= choice_idx < len(moves):
                    hand_idx, _, needs_rotation = moves[choice_idx]
