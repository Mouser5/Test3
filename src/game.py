...
from cards import (TunnelCardTemplate, StartCardTemplate, DoorCardTemplate,
                   ...
                   ActionType, EquipmentType, Direction)
from actions import AgentAction, ActionBuild, ActionPlayBoardUtility, ActionPlayPlayerUtility, ActionDiscard
...
class Game:
    ...
    def _build_decks(self):
        ...
        counts = {
            ...
            "tunnel_bridge": 4,"tunnel_double_corner": 4,"tunnel_split_t_up": 4,"tunnel_split_t_l":4, "door_blue": 3, "door_green": 3, "ladder": 4,
    ...
    def _handle_build(self, action: ActionBuild) -> Tuple[bool, str, Optional[int]]:
        ...
        if not self.board_engine.is_move_valid(action.x, action.y, placed, self.start_positions[p_id], p_id,
        ...
        return True, f"Игрок {p_id} построил на ({action.x}, {action.y}).", revealed_gold
    ...
    def _handle_board_utility(self, action: ActionPlayBoardUtility) -> Tuple[bool, str, Optional[int]]:
        ...
        if template.action_type == ActionType.KEY:
            ...
            if not self.board_engine.is_move_valid(action.x, action.y, PlacedCard(template_id=action.template_id),
    ...
    def get_legal_actions(self) -> List[AgentAction]:
        ...
        for tpl_tuple in set(combinations(sorted(player_state.hand), 2)):
            ...
            for eq in player_state.broken_equipments:
                legal_actions.append(ActionDiscard(templates=list(tpl_tuple), repair_equipment=eq))
                ...
                if not player_state.broken_equipments:
                    ...
                    for x, y in frontier_coords:
                        for is_rot in [False, True]:
                            placed = PlacedCard(template_id=t_id, owner_id=p_id, is_rotated_180=is_rot)
                            if isinstance(template, DoorCardTemplate):
                                placed.is_locked = True

                            # ИСПРАВЛЕНИЕ: Геометрическая проверка за O(1), BFS отключен!
                            if self.board_engine.is_move_valid(
                                    x, y, placed, self.start_positions[p_id], p_id,
            ...
            # 3. УТИЛИТАРНЫЕ КАРТЫ НА ПОЛЕ (Ключ, Обвал, Карта Сокровищ)
                ...
                for coord_key, target_placed in self.state.board.items():
                    ...
                    if template.action_type == ActionType.KEY:
                        ...
                        if self.board_engine.is_move_valid(tx, ty, PlacedCard(template_id=t_id),
                                                           self.start_positions[p_id], p_id, self.state.board,
                                                           player_state.ladders):
                            legal_actions.append(ActionPlayBoardUtility(template_id=t_id, x=tx, y=ty))
        ...
        for coord_key, placed_card in self.state.board.items():
            ...
            if isinstance(tpl, GoldCardTemplate) and not placed_card.is_revealed:
                if coord_key not in player_secrets:
                    ...
                    obs_board[coord_key] = placed_card.model_copy(update={"template_id": "hidden_gold"})
                    continue
